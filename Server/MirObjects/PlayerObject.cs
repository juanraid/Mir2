...
namespace Server.MirObjects
    ...
    public sealed class PlayerObject : MapObject
        ...
        public long LastRecallTime, LastRevivalTime, LastTeleportTime, LastProbeTime, MenteeEXP, LastItemUsed;
        ...
        public void StopGame(byte reason)
            ...
            DateTime theDate = Info.LastDate;

            string sqlAccountLast = "UPDATE " + Settings.DBAccount + ".characterinfo SET LastDate = '" + theDate.ToString("yyyy-MM-dd H:mm:ss") + "', LastIP = '" + Info.LastIP + "'  WHERE IndexID = '" + Info.Index + "'";
            ...
            List<AccountInfo> AccountClose = new List<AccountInfo>();
            AccountClose.Add(Info.AccountInfo);

            _threadCloseDB = new Thread(() => Envir.ConnectADB.SaveProgressDB(AccountClose));
            _threadCloseDB.IsBackground = true;
            _threadCloseDB.Start();

            if (reason != 23)
                {
                Envir.RemoveAccountList(Info.AccountInfo.Index);

                for (int i = 0; i < Info.AccountInfo.Characters.Count; i++)
                    {
                    Envir.RemoveCharterList(Info.AccountInfo.Characters[i].Index);
        ...
        private void NewCharacter()
            ...
            string sqlNewCharacter = "UPDATE " + Settings.DBAccount + ".characterinfo SET Hair = '" + Hair + "', Level = '" + Level + "'  WHERE IndexID = '" + Info.Index + "'";

            Envir.ConnectADB.Update(sqlNewCharacter);
            ...
            if (Envir.Time > TorchTime)
                ...
                if (item != null)
                    ...
                    if (item.CurrentDura == 0)
                        {
                        string sqlDelete = "DELETE FROM " + Settings.DBAccount + ".inventory WHERE UniqueID = '" + Info.Equipment[(int)EquipmentSlot.Torch].UniqueID + "'";
        ...
        private void ProcessBuffs()
            ...
            for (int i = Buffs.Count - 1; i >= 0; i--)
                ...
                if (buff.Type == BuffType.Drop || buff.Type == BuffType.Exp || buff.Type == BuffType.General || buff.Type == BuffType.Transform)
                    {
                    int typeBuff = Convert.ToInt32(buff.Type);

                    string sqlDelete = "DELETE FROM " + Settings.DBAccount + ".buff WHERE Type = '" + typeBuff + "' AND ChName = '" + Info.Name + "'; DELETE FROM " + Settings.DBAccount + ".buff_value WHERE Type = '" + typeBuff + "' AND ChName = '" + Info.Name + "'";
        ...
        private void ProcessItems()
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
                ...
                if (DateTime.Now > item.ExpireInfo.ExpiryDate)
                    ...
                    DeleteItemDB(Info.Inventory[i].UniqueID, "inventory", true);
            ...
            for (int i = 0; i < Info.Equipment.Length; i++)
                ...
                if (DateTime.Now > item.ExpireInfo.ExpiryDate)
                    ...
                    DeleteItemDB(Info.Equipment[i].UniqueID, "inventory", true);
        ...
        public override void Die()
            ...
            if (HasRevivalRing && Envir.Time > LastRevivalTime)
                ...
                for (var i = (int)EquipmentSlot.RingL; i <= (int)EquipmentSlot.RingR; i++)
                    ...
                    string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET CurrentDura = '" + item.CurrentDura + "'  WHERE UniqueID = '" + item.UniqueID + "'";

                    Envir.ConnectADB.Update(UpdateItem);
            ...
            if (LastHitter != null && LastHitter.Race == ObjectType.Player)
                ...
                if (AtWar(hitter) || WarZone)
                    ...
                    if (weapon != null && weapon.Luck > (Settings.MaxLuck * -1) && Envir.Random.Next(4) == 0)
                        ...
                        string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET Luck = '" + weapon.Luck + "'  WHERE UniqueID = '" + weapon.UniqueID + "'";
                ...
                for (int i = 0; i < Info.Equipment.Length; i++)
                    ...
                    if (temp.Count > 1)
                        ...
                        if (DropItem(temp2, Settings.DropRange, true))
                            ...
                            string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET Count = '" + temp.Count + "'  WHERE UniqueID = '" + temp.UniqueID + "'";
        ...
        public void GainExp(uint amount)
            ...
            if (Info.Married != 0)
                ...
                if (buff != null)
                    ...
                    PlayerObject player = Envir.GetPlayer(Info.MarriedName);
            ...
            if (Info.Mentor != 0 && !Info.isMentor)
                ...
                if (buffMentor != null)
                    ...
                    PlayerObject player = Envir.GetPlayer(Info.MentorName);
                    if (player != null && player.CurrentMap == CurrentMap && Functions.InRange(player.CurrentLocation, CurrentLocation, Globals.DataRange) && !player.Dead)
                        {
            ...
            while (experience >= MaxExperience)
                ...
                string UpdateItem = "UPDATE " + Settings.DBAccount + ".characterinfo SET Level = '" + Level + "', Experience = '" + Convert.ToInt32(experience) + "' WHERE IndexID = '" + Info.Index + "'";
        ...
        private void AddQuestItem(UserItem item)
            ...
            for (int i = 0; i < Info.QuestInventory.Length; i++)
                ...
                if (item.Count + temp.Count <= temp.Info.StackSize)
                    ...
                    sqlCount = "UPDATE " + Settings.DBAccount + ".inventory SET  Count = '" + temp.Count + "'  WHERE UniqueID = '" + temp.UniqueID + "'";
                    Envir.ConnectADB.Update(sqlCount);
                   // UserItem clonedItem = temp.Clone();
                  //  Enqueue(new S.GainedQuestItem { Item = temp });
                    return;
                    }
            ...
            for (int i = 0; i < Info.QuestInventory.Length; i++)
                ...
                item.UniqueID = ++Envir.NextUserItemID;
                Info.QuestInventory[i] = item;

                string sqlCount = "UPDATE " + Settings.DBAccount + ".generalcount SET  NextUserItemID = '" + Envir.NextUserItemID + "'  WHERE IndexID = '1'";
                Envir.ConnectADB.Update(sqlCount);

                GainAddItemDB(Info.QuestInventory[i], "inventory", "QuestInventory", i);
                return;
                }

            }

        private void InsertItemStorage(UserItem item, string store)
            { }
        ...
        private void AddItem(UserItem item)
            ...
            if (item.Info.Type == ItemType.Potion || item.Info.Type == ItemType.Scroll || (item.Info.Type == ItemType.Script && item.Info.Effect == 1))
                ...
                for (int i = 0; i < 4; i++)
                    ...
                    GainAddItemDB(item, "inventory", "Inventory", i);
        ...
        public void DeleteItemDB(ulong UniqueID, string table, bool awake)
            {
            string sqlDelete = "DELETE FROM " + Settings.DBAccount + "." + table + " WHERE UniqueID = '" + UniqueID + "' OR Attached = '" + UniqueID + "'";
            ...
            if (awake)
                ...
                string sqlDeleteAwake = "DELETE FROM " + Settings.DBAccount + ".awake WHERE UniqueID = '" + UniqueID + "'";

                Envir.ConnectADB.Delete(sqlDeleteAwake);
                }
            }
        private String AddItemDBString(UserItem item, string table, string store, int position, ulong IdIndex = 0)
            {
            ...
            if (table.ToLower() == "storage")
                {
                ID = "AccountID";
                IdValue = Info.AccountInfo.AccountID;
                }
            else
                {
                ID = "ChName";
        ...
        public void GainAddItemDB(UserItem clonedItem, string table, string store, int position, ulong IdIndex = 0)
            {
            string sqlInsertDB;
            ...
            sqlInsertDB = AddItemDBString(clonedItem, table, store, position, IdIndex);
            ...
            if (clonedItem.IsAttached == true)
                ...
                for (int j = 0; j < clonedItem.Slots.Length; j++)
                    ...
                    CheckItem(clonedItem.Slots[j]);

                    UserItem clonedItem1 = clonedItem.Slots[j].Clone();

                    sqlInsertDB = AddItemDBString(clonedItem1, table, typestore, j, IdIndex);
        ...
        private void AddItemDB(UserItem item, string table, string store, int position, ulong IdIndex = 0)
            {
            string sqlInsertDB;
        ...
        private void SetBind()
            ...
            /*
                        string sqlBindMap = "UPDATE " + Settings.DBAccount + ".characterinfo SET BindMapIndex = '" + BindMapIndex + "', BindLocation_X = '" + BindLocation.X + "', BindLocation_Y = '" + BindLocation.Y + "' WHERE IndexID = '" + this.Info.Index + "'";

                        Envir.ConnectADB.Update(sqlBindMap);
            */
            }
        ...
        private void StartGameSuccess()
            ...
            ReceiveChat(".Bienvenido a Legen of Mir Alpha.", ChatType.Hint);
        ...
        //revisar aumentar stats hay que modificar en el cliente misma funcion
        ...
        private void AddTempSkills(IEnumerable<string> skillsToAdd)
            ...
            foreach (var skill in skillsToAdd)
                ...
                //  string sql = "INSERT INTO " + Settings.DBAccount + ".magics (ChName, Spell, Level, Key_, Experience, IsTempSpell, CastTime) VALUES ('"+ Info.Name + "', '"+magic.Spell + "', '" + magic.Level + "', '" + magic.Key + "', '" + magic.Experience + "', '" + magic.IsTempSpell + "', '" + magic.CastTime+ "')";
                // Envir.ConnectADB.Insert(sql);
            ...
            if (message.StartsWith("/"))
                ...
                switch (parts[0].ToUpper())
                    ...
                    case "RESTORE":
                        ...
                        data = Envir.GetCharacterInfoDB(parts[1]);
                        ...
                        string Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET Deleted = '" + data.Deleted + "'  WHERE IndexID = '" + data.Index + "'";
                    ...
                    case "CHANGEGENDER":
                        ...
                        Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET Gender = '" + Convert.ToInt32(data.Gender) + "'  WHERE IndexID = '" + data.Index + "'";
                    ...
                    case "LEVEL":
                        ...
                        if (parts.Length >= 3)
                            ...
                            if (ushort.TryParse(parts[2], out level))
                                ...
                                Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET Level = '" + player.Level + "'  WHERE IndexID = '" + player.Info.Index + "'";
                            ...
                            if (ushort.TryParse(parts[1], out level))
                                ...
                                Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET Level = '" + Info.Level + "' WHERE IndexID = '" + Info.Index + "'";
                    ...
                    case "CLEARBAG":
                        ...
                        for (int i = 0; i < player.Info.Inventory.Length; i++)
                            ...
                            if (item == null) continue;
                            DeleteItemDB(player.Info.Inventory[i].UniqueID, "inventory", true);
                    ...
                    case "GIVESKILL":
                        ...
                        if (player.Info.Magics.Any(e => e.Spell == skill))
                            ...
                            int IdSpell = Convert.ToInt32(magic.Spell);

                            string sqlmagic = "UPDATE  " + Settings.DBAccount + ".magics  SET Level = '" + magic.Level + "', Experience = '0' WHERE ChName = '" + Info.Name + "' AND Spell = '" + IdSpell + "'";

                            Envir.ConnectADB.Insert(sqlmagic);
                            return;
                            }
                        else
                            {
                            ...
                            string sqlSpell = "INSERT INTO " + Settings.DBAccount + ".magics (ChName, Spell, Level, Key_, Experience, IsTempSpell, CastTime) VALUES ('" + Info.Name + "', '" + IdSpell + "', '" + magic.Level + "', '" + magic.Key + "', '" + magic.Experience + "', '" + magic.IsTempSpell + "', '" + magic.CastTime + "')";

                            Envir.ConnectADB.Insert(sqlSpell);
                            }
                    ...
                    case "ALLOWTRADE":
                        ...
                        string UpdateTrade = "UPDATE " + Settings.DBAccount + ".characterinfo SET AllowTrade = '" + Convert.ToInt32(AllowTrade) + "'  WHERE IndexID = '" + Info.Index + "'";

                        Envir.ConnectADB.Update(UpdateTrade);
                    ...
                    case "SETFLAG":
                        ...
                        if (!Info.Flags[tempInt])
                            {
                            sql = "DELETE FROM " + Settings.DBAccount + ".flags where Number = '" + tempInt + "' And ChName = '" + Info.Name + "'";
                            Envir.ConnectADB.Delete(sql);
                            }
                        else
                            {
                            sql = "INSERT INTO " + Settings.DBAccount + ".flags (Flags, Number, ChName) VALUES ('1', '" + tempInt + "', '" + Info.Name + "')";
                    ...
                    case "CLEARFLAGS":
                        ...
                        for (int i = 0; i < player.Info.Flags.Length; i++)
                            ...
                            string clear = "DELETE FROM " + Settings.DBAccount + ".flags where  ChName = '" + Info.Name + "'";
                            Envir.ConnectADB.Delete(clear);
                            }
                        break;
                    case "CLEARMOB":
                        if (!IsGM) return;
                    ...
                    case "CHANGECLASS": //@changeclass [Player] [Class]
                        ...
                        Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET Class = '" + Convert.ToInt32(data.Class) + "'  WHERE IndexID = '" + data.Index + "'";
                    ...
                    case "HAIR":
                        ...
                        Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET Hair = '" + Info.Hair + "'  WHERE IndexID = '" + Info.Index + "'";
                    ...
                    case "ADJUSTPKPOINT":
                        ...
                        Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET PKPoints = '" + player.PKPoints + "'  WHERE IndexID = '" + player.Info.Index + "'";
                                ...
                                if (realItem.Type == type)
                                    ...
                                    switch (result)
                                        ...
                                        case 1:
                                            ...
                                            Enqueue(p);
                                            int position = temp.Awake.listAwake.Count - 1;

                                            string sqlAwake = "INSERT INTO " + Settings.DBAccount + ".awake (UniqueID, Value, Position) VALUES ('" + temp.UniqueID + "', '" + temp.Awake.listAwake[position] + "', '" + position + "')";

                                            Envir.ConnectADB.Insert(sqlAwake);

                                            if (position == 0)
                                                {
                                                temp.AwakeType = Convert.ToInt32(temp.Awake.type);
                                                temp.IsAwake = true;

                                                string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET AwakeType = '" + Convert.ToInt32(temp.Awake.type) + "', IsAwake = '1'  WHERE UniqueID = '" + temp.UniqueID + "'";
                                ...
                                if (realItem.Type == type)
                                    ...
                                    switch (result)
                                        ...
                                        case 1:
                                            ...
                                            int position = temp.Awake.listAwake.Count;

                                            string sqlDelete = "DELETE FROM " + Settings.DBAccount + ".awake WHERE UniqueID = '" + temp.UniqueID + "' and Position = '" + position + "'";
                                            ...
                                            if (position == 0)
                                                ...
                                                temp.AwakeType = Convert.ToInt32(temp.Awake.type);
                                                temp.IsAwake = false;
                                                string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET AwakeType = '0', IsAwake = ''  WHERE UniqueID = '" + temp.UniqueID + "'";
                    ...
                    case "ADDINVENTORY":
                            {
                            if (Info.Inventory.Length >= 86)
                                {
                                ReceiveChat("Maximum Inventory reached.", ChatType.System);
                                return;
                                }
                    ...
                    case "ADDSTORAGE":
                            {
                            if (Account.Storage.Length >= 160)
                                {
                                ReceiveChat("Maximum Storage reached.", ChatType.System);
                                return;
                                }
                    ...
                    case "SETQUEST":
                        ...
                        if (activeQuest != null)
                            ...
                            string sqlDelete = "DELETE FROM " + Settings.DBAccount + ".currentquests WHERE IndexID = '" + activeQuest.Index + "' and ChName = '" + Info.Name + "'; DELETE FROM " + Settings.DBAccount + ".flagtaskset WHERE IndexID = '" + activeQuest.Index + "' and ChName = '" + Info.Name + "'; DELETE FROM " + Settings.DBAccount + ".itemtaskcount WHERE IndexID = '" + activeQuest.Index + "' and ChName = '" + Info.Name + "'; DELETE FROM " + Settings.DBAccount + ".killtaskcount WHERE IndexID = '" + activeQuest.Index + "' and ChName = '" + Info.Name + "'";
                        ...
                        switch (questState)
                            ...
                            case 1: //complete
                                ...
                                QuestInfo quest = Envir.QuestInfoList.FirstOrDefault(d => d.Index == questid);
                                sql = "INSERT INTO " + Settings.DBAccount + ".completedquests (ChName, CompletedQuests, Quest, Type, Finish) VALUES ('" + Info.Name + "', '" + quest.Index + "', '" + quest.Name + "', '" + quest.Type.ToString() + "', '" + theDate.ToString("yyyy-MM-dd H:mm:ss").ToString() + "')";
            ...
            switch (magic.Spell)
                ...
                case Spell.Entrapment:
                    ...
                    if (target == null || !target.IsAttackTarget(this) || target.CurrentMap != CurrentMap || target.Node == null || target.Race != ObjectType.Monster ||
                        Functions.MaxDistance(CurrentLocation, target.CurrentLocation) > 7 || target.Level >= Level + 5 + Envir.Random.Next(8))
                        return;
                ...
                case Spell.Hallucination:
                    ...
                    if (target == null || !target.IsAttackTarget(this) || target.CurrentMap != CurrentMap || target.Node == null ||
                        Functions.MaxDistance(CurrentLocation, target.CurrentLocation) > 7 || Envir.Random.Next(Level + 20 + magic.Level * 5) <= target.Level + 10)
                        return;
                    item = GetAmulet(1);
                    if (item == null) return;
        ...
        private void DeleteFishingItem(FishingSlot type)
            ...
            DeleteItemDB(Info.Equipment[(int)EquipmentSlot.Weapon].Slots[(int)type].UniqueID, "inventory", false);
            ...
            switch (magic.Level)
                ...
                case 0:
                    ...
                    if (magic.Experience >= magic.Info.Need1)
                        ...
                        string RemoveMagic = "UPDATE " + Settings.DBAccount + ".magics SET Level = '" + magic.Level + "', Experience = '" + magic.Experience + "'  WHERE ChName = '" + Info.Name + "' and Spell = '" + magic + "'";

                        Envir.ConnectADB.Update(RemoveMagic);
                        RefreshStats();
                        }
                    break;
                case 1:
        ...
        public override void AddBuff(Buff b)
            ...
            if (b.Type == BuffType.Drop || b.Type == BuffType.Exp || b.Type == BuffType.General || b.Type == BuffType.Transform)
                {

                int typeBuff = Convert.ToInt32(b.Type);

                string sqlCountBuff = "SELECT Count(*) FROM " + Settings.DBAccount + ".buff where Type = '" + typeBuff + "' And ChName = '" + Info.Name + "'";
                if (Envir.ConnectADB.Count(sqlCountBuff) > 0)
                    {
                    var sqlbuff = "UPDATE  " + Settings.DBAccount + ".buff  SET ExpireTime = '" + b.ExpireTime + "' WHERE ChName = '" + Info.Name + "' AND Type = '" + typeBuff + "'";

                    Envir.ConnectADB.Insert(sqlbuff);

                    for (int va = 0; va < b.Values.Length; va++)
                        {

                        var sqlbuff_value = "UPDATE  " + Settings.DBAccount + ".buff_value SET Value = '" + b.Values[va] + "' WHERE ChName = '" + Info.Name + "' AND Type = '" + typeBuff + "' And Position = '" + va + "'";

                        Envir.ConnectADB.Insert(sqlbuff_value);
                    ...
                    var sqlbuff = "INSERT INTO  " + Settings.DBAccount + ".buff ( Type, Visible, ChName, ObjectID, ExpireTime, Infinite) VALUES ('" + typeBuff + "', '" + b.Visible + "', '" + Info.Name + "', '" + b.ObjectID + "', '" + (b.ExpireTime - Envir.Time) + "', '" + b.Infinite + "')";
                    ...
                    for (int va = 0; va < b.Values.Length; va++)
                        ...
                        var sqlbuff_value = "INSERT INTO  " + Settings.DBAccount + ".buff_value (Value, Type, ChName, Position) VALUES ('" + b.Values[va] + "', '" + typeBuff + "', '" + Info.Name + "', '" + va + "')";
                        Envir.ConnectADB.Insert(sqlbuff_value);
        ...
        public void EquipSlotItem(MirGridType grid, ulong id, int to, MirGridType gridTo)
            ...
            if (CanUseItem(temp))
                ...
                if (grid == MirGridType.Inventory)
                    {
                    string AddSlotItem = "UPDATE " + Settings.DBAccount + ".inventory SET Store = '" + gridTo + "', Position = '" + to + "',  Attached = '" + Convert.ToUInt64(Item.UniqueID) + "' WHERE UniqueID = '" + id + "'; UPDATE " + Settings.DBAccount + ".inventory SET IsAttached = '1' WHERE UniqueID = '" + Item.UniqueID + "'";

                    Envir.ConnectADB.Update(AddSlotItem);
                ...
                if (grid == MirGridType.Storage)
                    ...
                    AddItemDB(Item.Slots[to], "inventory", gridTo.ToString(), to);

                    DeleteItemDB(id, "storage", false);

                    }
                Item.IsAttached = true;
                Item.Slots[to].Attached = Item.UniqueID;
                array[index] = null;
                p.Success = true;
                Enqueue(p);
                RefreshStats();
        ...
        public void RemoveItem(MirGridType grid, ulong id, int to)
            ...
            if (array[to] == null)
                ...
                if (grid == MirGridType.Inventory)
                    {
                    string RemoveItem = "UPDATE " + Settings.DBAccount + ".Inventory SET Store = '" + grid + "', Position = '" + to + "',  CurrentDura = '" + array[to].CurrentDura + "' WHERE UniqueID = '" + id + "'";
                ...
                if (grid == MirGridType.Storage)
                    {
                    AddItemDB(array[to], "storage", grid.ToString(), to);
                    DeleteItemDB(id, "inventory", false);
        ...
        public void RemoveSlotItem(MirGridType grid, ulong id, int to, MirGridType gridTo)
            ...
            if (gridTo == MirGridType.Inventory)
                {
                string AddSlotItem = "UPDATE " + Settings.DBAccount + ".inventory SET Store = '" + gridTo + "', Position = '" + to + "',  Attached = '0' WHERE UniqueID = '" + id + "'";
            ...
            if (gridTo == MirGridType.Storage)
                {
                temp.Slots[index].Attached = 0;

                AddItemDB(temp.Slots[index], "storage", gridTo.ToString(), to);
            ...
            if (checkSlots == false)
                ...
                string AddSlotItem = "UPDATE " + Settings.DBAccount + ".inventory SET IsAttached = '0' WHERE UniqueID = '" + temp.UniqueID + "'";
        ...
        public void MoveItem(MirGridType grid, int from, int to)
            ...
            if (from >= 0 && to >= 0 && from < array.Length && to < array.Length)
                ...
                if (grid == MirGridType.Storage)
                    ...
                    string sqlDeleteInventory = "UPDATE " + Settings.DBAccount + ".storage SET Store = '" + grid + "', Position = '" + to + "'  WHERE UniqueID = '" + array[to].UniqueID + "'";

                    if (array[from] != null)
                        {
                        sqlDeleteInventory += "; UPDATE " + Settings.DBAccount + ".storage SET Store = '" + grid + "', Position = '" + from + "'  WHERE UniqueID = '" + array[from].UniqueID + "'";
                        }

                    Envir.ConnectADB.Update(sqlDeleteInventory);
                    }
                else
                    {
                    string sqlMoveItem = "UPDATE " + Settings.DBAccount + ".inventory SET Store = '" + grid + "', Position = '" + to + "'  WHERE UniqueID = '" + array[to].UniqueID + "'";

                    if (array[from] != null)
                        {
                        sqlMoveItem += "; UPDATE " + Settings.DBAccount + ".inventory SET Store = '" + grid + "', Position = '" + from + "'  WHERE UniqueID = '" + array[from].UniqueID + "'";
                        }

                    Envir.ConnectADB.Update(sqlMoveItem);
        ...
        public void StoreItem(int from, int to)
            ...
            if (Account.Storage[to] == null)
                ...
                AddItemDB(Account.Storage[to], "storage", "Storage", to);

                DeleteItemDB(Info.Inventory[from].UniqueID, "inventory", false);
        ...
        public void TakeBackItem(int from, int to)
            ...
            if (Info.Inventory[to] == null)
                ...
                AddItemDB(Info.Inventory[to], "inventory", "Inventory", to);

                DeleteItemDB(Account.Storage[from].UniqueID, "storage", false);
        ...
        public void EquipItem(MirGridType grid, ulong id, int to)
            ...
            if (CanEquipItem(temp, to))
                ...
                if (grid == MirGridType.Storage)
                    ...
                    if (array[index] != null)
                        {
                        AddItemDB(array[index], "storage", "Storage", index);

                        DeleteItemDB(array[index].UniqueID, "inventory", false);

                        }

                    Info.Equipment[to] = temp;

                    AddItemDB(Info.Equipment[to], "inventory", "Equipment", to);

                    DeleteItemDB(Info.Equipment[to].UniqueID, "storage", false);
                ...
                if (grid == MirGridType.Inventory)
                    ...
                    if (Info.Equipment[to] != null)
                        ...
                        string sqlRemoveEquip = "UPDATE " + Settings.DBAccount + ".inventory SET Store = 'Inventory', Position = '" + index + "'  WHERE UniqueID = '" + Info.Equipment[to].UniqueID + "'";

                        Envir.ConnectADB.Update(sqlRemoveEquip);
                        }

                    Info.Equipment[to] = temp;

                    string sqlRemoveItem = "UPDATE " + Settings.DBAccount + ".inventory SET Store = 'Equipment', Position = '" + to + "'  WHERE UniqueID = '" + Info.Equipment[to].UniqueID + "'";

                    Envir.ConnectADB.Update(sqlRemoveItem);
        ...
        public void UseItem(ulong id)
            ...
            if ((Envir.Time < LastItemUsed))
                {
                ReceiveChat(string.Format("You cannot use for another {0} seconds", (LastItemUsed - Envir.Time) / 1000), ChatType.System);
                Enqueue(p);
                return;
                }

            LastItemUsed = Envir.Time + 1800;
            ...
            switch (item.Info.Type)
                ...
                case ItemType.Scroll:
                    ...
                    switch (item.Info.Shape)
                        ...
                        case 4: //RepairOil
                            ...
                            string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET MaxDura = '" + temp.MaxDura + "', CurrentDura = '" + temp.CurrentDura + "'  WHERE UniqueID = '" + temp.UniqueID + "'";
                        ...
                        case 5: //WarGodOil
                            ...
                            string UpdateCurrentDura = "UPDATE " + Settings.DBAccount + ".inventory SET MaxDura = '" + temp.MaxDura + "', CurrentDura = '" + temp.CurrentDura + "'  WHERE UniqueID = '" + temp.UniqueID + "'";

                            Envir.ConnectADB.Update(UpdateCurrentDura);
                ...
                case ItemType.Book:
                    ...
                    string sql = "INSERT INTO " + Settings.DBAccount + ".magics (ChName, Spell, Level, Key_, Experience, IsTempSpell, CastTime) VALUES ('" + Info.Name + "', '" + IdSpell + "', '" + magic.Level + "', '" + magic.Key + "', '" + magic.Experience + "', '" + magic.IsTempSpell + "', '" + magic.CastTime + "')";
                    Envir.ConnectADB.Insert(sql);
                ...
                case ItemType.Food:
                    ...
                    switch (item.Info.Shape)
                        ...
                        case 0:
                            ...
                            string UpdateMoun = "UPDATE " + Settings.DBAccount + ".inventory SET CurrentDura = '" + temp.CurrentDura + "'  WHERE UniqueID = '" + temp.UniqueID + "'";
                            Envir.ConnectADB.Update(UpdateMoun);
                            break;
                        case 1:
                            break;
                        }
                    ...
                    string UpdateMount = "UPDATE " + Settings.DBAccount + ".inventory SET CurrentDura = '" + temp.CurrentDura + "'  WHERE UniqueID = '" + temp.UniqueID + "'";

                    Envir.ConnectADB.Update(UpdateMount);
                ...
                case ItemType.Pets:
                    if (item.Info.Shape >= 20)
                        ...
                        switch (item.Info.Shape)
                            ...
                            case 21://BlackStone
                                if (item.Count > 1)
                                    {
                                    item.Count--;
                                    string UpdateStone = "UPDATE " + Settings.DBAccount + ".inventory SET Count = '" + item.Count + "'  WHERE UniqueID = '" + item.UniqueID + "'";

                                    Envir.ConnectADB.Update(UpdateStone);
                                    }
                                else
                                    {
                                    DeleteItemDB(item.UniqueID, "inventory", false);
                            ...
                            case 25://Strongbox
                                ...
                                if (item.Count > 1)
                                    ...
                                    DeleteItemDB(Info.Inventory[index].UniqueID, "inventory", false);
                                    Info.Inventory[index] = null;
                        ...
                        string sqlCreatures = "INSERT INTO " + Settings.DBAccount + ".intelligentcreatures (ChName, PetType, CustomName, Fullness, SlotIndex, ExpireTime, BlackstoneTime, MaintainFoodTime,PetTypeName,PetMode) VALUES ('" + Info.Name + "', '" + item.Info.Shape + "', '" + petInfo.CustomName + "', '" + petInfo.Fullness + "', '" + petInfo.SlotIndex + "', '" + petInfo.ExpireTime + "', '" + petInfo.BlackstoneTime + "', '" + petInfo.MaintainFoodTime + "', '" + petInfo.PetType + "', '" + petInfo.petMode + "')";

                        Envir.ConnectADB.Insert(sqlCreatures);
            ...
            if (item.Count > 1)
                {
                item.Count--;
                string sqlCount = "UPDATE " + Settings.DBAccount + ".inventory SET  Count = '" + item.Count + "'  WHERE UniqueID = '" + item.UniqueID + "'";
        ...
        public void SplitItem(MirGridType grid, ulong id, uint count)
            ...
            string RemoveItem = "UPDATE " + Settings.DBAccount + "." + grid + " SET  Count = '" + temp.Count + "'  WHERE UniqueID = '" + p.UniqueID + "'";
            ...
            if (grid == MirGridType.Inventory && (temp.Info.Type == ItemType.Potion || temp.Info.Type == ItemType.Scroll || temp.Info.Type == ItemType.Amulet || (temp.Info.Type == ItemType.Script && temp.Info.Effect == 1)))
                ...
                if (temp.Info.Type == ItemType.Potion || temp.Info.Type == ItemType.Scroll || (temp.Info.Type == ItemType.Script && temp.Info.Effect == 1))
                    ...
                    for (int i = 0; i < 4; i++)
                        ...
                        AddItemDB(array[i], grid.ToString(), grid.ToString(), i);
        ...
        public void MergeItem(MirGridType gridFrom, MirGridType gridTo, ulong fromID, ulong toID)
            ...
            if (tempFrom.Count <= tempTo.Info.StackSize - tempTo.Count)
                ...
                if (gridTo == MirGridType.Equipment || gridTo == MirGridType.Fishing)
                    {
                    gridTo = MirGridType.Inventory;
                    }
                if (gridFrom == MirGridType.Equipment || gridFrom == MirGridType.Fishing)
                    {
                    gridFrom = MirGridType.Inventory;

                    }
                string sqlUpdateStorage = "UPDATE " + Settings.DBAccount + "." + gridTo + " SET  Count = '" + tempTo.Count + "'  WHERE UniqueID = '" + tempTo.UniqueID + "'";

                Envir.ConnectADB.Delete(sqlUpdateStorage);

                DeleteItemDB(arrayFrom[index].UniqueID, gridFrom.ToString(), false);
                ...
                string sqlUpdate = "UPDATE " + Settings.DBAccount + "." + gridFrom + " SET  Count = '" + tempFrom.Count + "'  WHERE UniqueID = '" + tempFrom.UniqueID + "'";

                Envir.ConnectADB.Delete(sqlUpdate);
        ...
        public void CombineItem(ulong fromID, ulong toID)
            ...
            switch (tempFrom.Info.Shape)
                ...
                case 4: //orbs
                    ...
                    if ((tempFrom.Info.MaxDC + tempFrom.DC) > 0)
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET DC = '" + tempTo.DC + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";

                        Envir.ConnectADB.Update(UpdateItemGem);
                        }
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET MC = '" + tempTo.MC + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET SC = '" + tempTo.SC + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET AC = '" + tempTo.AC + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET MAC = '" + tempTo.MAC + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET MaxDura = '" + tempTo.MaxDura + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET AttackSpeed = '" + tempTo.AttackSpeed + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET Agility = '" + tempTo.Agility + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET Accuracy = '" + tempTo.Accuracy + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET PoisonAttack = '" + tempTo.PoisonAttack + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET Freezing = '" + tempTo.Freezing + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET MagicResist = '" + tempTo.MagicResist + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
                        ...
                        string UpdateItemGem = "UPDATE " + Settings.DBAccount + ".inventory SET PoisonResist = '" + tempTo.PoisonResist + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
                    ...
                    if (!succeeded)
                        ...
                        if ((tempFrom.Info.Shape == 3) && (Envir.Random.Next(15) < 3))
                            ...
                            DeleteItemDB(Info.Inventory[indexTo].UniqueID, "inventory", false);
            ...
            if (canRepair && Info.Inventory[indexTo] != null)
                ...
                switch (tempTo.Info.Shape)
                    ...
                    case 2:
                            ...
                            string UpdateMaxDura = "UPDATE " + Settings.DBAccount + ".inventory SET MaxDura = '" + tempTo.MaxDura + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";

                            Envir.ConnectADB.Update(UpdateMaxDura);
                            }
                        break;
                    default:
                        break;
                ...
                string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET CurrentDura = '" + tempTo.MaxDura + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
            ...
            if (canUpgrade && Info.Inventory[indexTo] != null)
                ...
                string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET GemCount = '" + tempTo.GemCount + "' WHERE UniqueID = '" + tempTo.UniqueID + "'";
            ...
            if (tempFrom.Count > 1)
                ...
                tempFrom.Count--;
                string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET Count = '" + tempFrom.Count + "' WHERE UniqueID = '" + tempFrom.UniqueID + "'";
                ...
                DeleteItemDB(Info.Inventory[indexFrom].UniqueID, "inventory", false);
        ...
        public void DropItem(ulong id, uint count)
            ...
            if (temp.Count == count)
                ...
                DeleteItemDB(Info.Inventory[index].UniqueID, "inventory", true);
                ...
                string RemoveItem = "UPDATE " + Settings.DBAccount + ".inventory SET Count = '" + temp.Count + "'  WHERE UniqueID = '" + temp.UniqueID + "'";
        ...
        public void DropGold(uint gold)
            ...
            if (!ob.Drop(5)) return;
            Account.Gold -= gold;
            string Update = "UPDATE " + Settings.DBAccount + ".account SET Gold= '" + Account.Gold + "'  WHERE IndexID = '" + Account.Index + "'";
        ...
        public void GainCredit(uint credit)
            ...
            string RemoveItem = "UPDATE " + Settings.DBAccount + ".account SET Credit = '" + Account.Credit + "' WHERE IndexID = '" + Account.Index + "'";
        ...
        public void TakeQuestItem(ItemInfo uItem, uint count)
            ...
            for (int o = 0; o < Info.QuestInventory.Length; o++)
                ...
                if (count > item.Count)
                    ...
                    DeleteItemDB(Info.QuestInventory[o].UniqueID, "inventory", false);
                ...
                if (count == item.Count)
                    ...
                    {
                    item.Count -= count;
                    string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET Count = '" + item.Count + "'Store = 'QuestInventory', Position = '" + o + "'  WHERE UniqueID = '" + item.UniqueID + "'";
        ...
        private void ConsumeItem(UserItem item, uint cost)
            ...
            if (item.Count == 0)
                ...
                for (int i = 0; i < Info.Equipment.Length; i++)
                    ...
                    if (Info.Equipment[i] != null && Info.Equipment[i].Slots.Length > 0)
                        ...
                        for (int j = 0; j < Info.Equipment[i].Slots.Length; j++)
                            ...
                            DeleteItemDB(Info.Equipment[i].Slots[j].UniqueID, "inventory", false);
                ...
                {
                string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET Count = '" + item.Count + "'  WHERE UniqueID = '" + item.UniqueID + "'";
        ...
        public bool TryLuckWeapon()
            ...
            if (item.Luck > (Settings.MaxLuck * -1) && Envir.Random.Next(20) == 0)
                ...
                string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET Luck= '" + item.Luck + "'  WHERE UniqueID = '" + item.UniqueID + "'";
        ...
        public void SellItem(ulong uniqueID, uint count)
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
                ...
                if (temp.Info.StackSize > 1 && count != temp.Count)
                    ...
                    temp.Count -= count;
                    string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET Count = '" + temp.Count + "'  WHERE UniqueID = '" + item.UniqueID + "'";
        ...
        public void RepairItem(ulong uniqueID, bool special = false)
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
                ...
                UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET CurrentDura = '" + temp.CurrentDura + "', MaxDura = '" + temp.MaxDura + "'  WHERE UniqueID = '" + temp.UniqueID + "'; UPDATE " + Settings.DBAccount + ".Account SET Gold = '" + Account.Gold + "'  WHERE IndexID = '" + Account.Index + "'";
        ...
        public void ConsignItem(ulong uniqueID, uint price)
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
                ...
                string UpdateItem = "UPDATE " + Settings.DBAccount + ".generalcount SET NextAuctionID = '" + auction.AuctionID + "'  WHERE IndexID = '1'";
                //use store to send AuctionID

                Envir.ConnectADB.Update(UpdateItem);

                AddItemDB(temp, "auctionsitems", "Auction", 0, auction.AuctionID);

                string sql = "INSERT INTO " + Settings.DBAccount + ".auctions (AuctionID, CharacterIndex,  Price, NameSeller ) VALUES ('" + auction.AuctionID + "', '" + auction.CharacterIndex + "', '" + auction.Price + "', '" + auction.NameSeller + "')";
                ...
                Report.ItemChanged("ConsignItem", temp, temp.Count, 0);
        ...
        public void GetMarket(string name, ItemType type)
            ...
            MatchType = type;
            PageSent = 0;
            LinkedListNode<AuctionInfo> current = Envir.Auctions.First;

            while (current != null)
                {
                if (UserMatch == true)
                    {
                    if (this.Name == current.Value.NameSeller)
                        {
        ...
        public void MarketBuy(ulong auctionID)
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
                ...
                foreach (AuctionInfo auction in Envir.Auctions)
                    ...
                    if (auction.NameSeller == this.Name)
                        {
                    ...
                    string Update = "UPDATE " + Settings.DBAccount + ".auctions SET Sold = '" + Convert.ToInt32(auction.Sold) + "'  WHERE AuctionID = '" + auction.AuctionID + "'; UPDATE " + Settings.DBAccount + ".account SET Gold = '" + Account.Gold + "'  WHERE IndexID = '" + Account.Index + "'";
                    Envir.ConnectADB.Update(Update);
                    ...
                    string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET ChName= '" + Info.Name + "'  WHERE Attached = '" + auction.Item.UniqueID + "'";
                    Envir.ConnectADB.Update(UpdateItem);
        ...
        public void MarketGetBack(ulong auctionID)
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
                ...
                foreach (AuctionInfo auction in Envir.Auctions)
                    ...
                    if (auction.NameSeller != this.Name) continue;
                    ...
                    if (!auction.Sold || auction.Expired)
                        ...
                        string sqlDelete = "DELETE FROM " + Settings.DBAccount + ".auctions WHERE AuctionID = '" + auction.AuctionID + "'; DELETE FROM " + Settings.DBAccount + ".auctionsitems WHERE AuctionID = '" + auction.AuctionID + "'";
                    ...
                    string sqlDelete1 = "DELETE FROM " + Settings.DBAccount + ".auctions WHERE AuctionID = '" + auction.AuctionID + "'; DELETE FROM " + Settings.DBAccount + ".auctionsitems WHERE AuctionID = '" + auction.AuctionID + "'";

                    Envir.ConnectADB.Delete(sqlDelete1);
        ...
        public void Awakening(ulong UniqueID, AwakeType type)
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
                ...
                if (HasAwakeningNeedMaterials(item, type))
                    ...
                    string UpdateGold = "UPDATE " + Settings.DBAccount + ".account SET Gold = '" + Info.AccountInfo.Gold + "'  WHERE IndexID = '" + Info.AccountInfo.Index + "'";

                    Envir.ConnectADB.Update(UpdateGold);
                    ...
                    switch (awake.UpgradeAwake(item, type, out isHit))
                        ...
                        case 1:
                            ...
                            int position = Info.Inventory[i].Awake.listAwake.Count - 1;

                            string sqlAwake = "INSERT INTO " + Settings.DBAccount + ".awake (UniqueID, Value, Position) VALUES ('" + Info.Inventory[i].UniqueID + "', '" + Info.Inventory[i].Awake.listAwake[position] + "', '" + position + "')";

                            Envir.ConnectADB.Insert(sqlAwake);

                            if (position == 0)
                                {
                                Info.Inventory[i].AwakeType = Convert.ToInt32(Info.Inventory[i].Awake.type);
                                Info.Inventory[i].IsAwake = true;
                                string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET AwakeType = '" + Convert.ToInt32(Info.Inventory[i].Awake.type) + "', IsAwake = '1'  WHERE UniqueID = '" + Info.Inventory[i].UniqueID + "'";
                        ...
                        if (Info.AccountInfo.Gold >= item.DowngradePrice())
                            ...
                            switch (result)
                                ...
                                case 1:
                                    ...
                                    string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET CurrentDura = '" + Info.Inventory[i].CurrentDura + "', MaxDura = '" + Info.Inventory[i].MaxDura + "'  WHERE UniqueID = '" + Info.Inventory[i].UniqueID + "'";
                                    ...
                                    int position = Info.Inventory[i].Awake.listAwake.Count;

                                    string sqlDelete = "DELETE FROM " + Settings.DBAccount + ".awake WHERE UniqueID = '" + Info.Inventory[i].UniqueID + "' and Position = '" + position + "'";
                                    ...
                                    if (position == 0)
                                        {
                                        Info.Inventory[i].AwakeType = Convert.ToInt32(Info.Inventory[i].Awake.type);
                                        Info.Inventory[i].IsAwake = false;
                                        UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET AwakeType = '0', IsAwake = ''  WHERE UniqueID = '" + Info.Inventory[i].UniqueID + "'";
        ...
        public void DisassembleItem(ulong UniqueID)
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
                ...
                if (Info.AccountInfo.Gold >= item.DisassemblePrice())
                    ...
                    string UpdateItem = "UPDATE " + Settings.DBAccount + ".Account SET Gold = '" + Info.AccountInfo.Gold + "' WHERE IndexID = '" + Info.AccountInfo.Index + "'";
                ...
                if (item != null)
                    ...
                    if (item.UniqueID == UniqueID)
                        ...
                        if (Info.AccountInfo.Gold >= item.ResetPrice())
                            ...
                            string sqlreset = "UPDATE " + Settings.DBAccount + ".inventory SET CurrentDura = '" + Info.Inventory[i].CurrentDura + "', MaxDura = '" + Info.Inventory[i].MaxDura + "', Count = '" + Info.Inventory[i].Count + "', MC = '" + Info.Inventory[i].MC + "', SC = '" + Info.Inventory[i].SC + "', AC = '" + Info.Inventory[i].AC + "', DC = '" + Info.Inventory[i].DC + "', MAC = '" + Info.Inventory[i].MAC + "', Accuracy = '" + Info.Inventory[i].Accuracy + "', Agility = '" + Info.Inventory[i].Agility + "', HP = '" + Info.Inventory[i].HP + "', MP = '" + Info.Inventory[i].MP + "', AttackSpeed = '" + Info.Inventory[i].AttackSpeed + "', Luck = '" + Info.Inventory[i].Luck + "', SoulBoundId = '" + Info.Inventory[i].SoulBoundId + "', Identified = '" + Info.Inventory[i].Identified + "', Cursed = '" + Info.Inventory[i].Cursed + "', Strong = '" + Info.Inventory[i].Strong + "', MagicResist = '" + Info.Inventory[i].MagicResist + "', PoisonResist = '" + Info.Inventory[i].PoisonResist + "', HealthRecovery = '" + Info.Inventory[i].HealthRecovery + "', ManaRecovery = '" + Info.Inventory[i].ManaRecovery + "', PoisonRecovery = '" + Info.Inventory[i].PoisonRecovery + "', CriticalRate = '" + Info.Inventory[i].CriticalRate + "', CriticalDamage = '" + Info.Inventory[i].CriticalDamage + "', Freezing = '" + Info.Inventory[i].Freezing + "', PoisonAttack = '" + Info.Inventory[i].PoisonAttack + "', Slots = '" + Info.Inventory[i].Slots + "', GemCount = '" + Info.Inventory[i].GemCount + "', RefinedValue = '" + Info.Inventory[i].RefinedValue + "', RefineAdded = '" + Info.Inventory[i].RefineAdded + "', WeddingRing = '" + Info.Inventory[i].WeddingRing + "', ExpireInfo = '" + Info.Inventory[i].ExpireInfo + "', ItemName = '" + Info.Inventory[i].Info.Name + "', IsAttached = '" + Info.Inventory[i].IsAttached + "', IsAwake = '" + Info.Inventory[i].IsAwake + "', AwakeType = '" + Info.Inventory[i].AwakeType + "' WHERE UniqueID = '" + Info.Inventory[i].UniqueID + "'";

                            Envir.ConnectADB.Update(sqlreset);
                    ...
                    if (Info.Inventory[i].Info.Grade == item.Info.Grade &&
                        ...
                        if (Info.Inventory[i].Info.Shape == ((int)type - 1) &&
                            ...
                            if (Info.Inventory[i].Count <= currentCount[0])
                                ...
                                string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET Count = '" + Info.Inventory[i].Count + "'  WHERE UniqueID = '" + Info.Inventory[i].UniqueID + "'";
        ...
        public void SwitchGroup(bool allow)
            ...
            string UpdateGroup = "UPDATE " + Settings.DBAccount + ".characterinfo SET AllowGroup = '" + Convert.ToInt32(allow) + "'  WHERE IndexID = '" + Info.Index + "'";

            Envir.ConnectADB.Update(UpdateGroup);
        ...
        public void CreateNewbieGuild(string GuildName)
            ...
            string UpdateItem = "UPDATE " + Settings.DBAccount + ".generalcount SET Guildindex = '" + Envir.NextGuildID + "'  WHERE IndexID = '1'";
            ...
            for (int i = 0; i < Settings.Guild_CreationCostList.Count; i++)
                ...
                if (Required.Item == null)
                    ...
                    if (Info.AccountInfo.Gold >= Required.Amount)
                        ...
                        string Update = "UPDATE " + Settings.DBAccount + ".account SET Gold = '" + Info.AccountInfo.Gold + "'  WHERE IndexID = '" + Account.Index + "'";
                        Envir.ConnectADB.Update(Update);
                    ...
                    for (int o = 0; o < Info.Inventory.Length; o++)
                        ...
                        if ((Required.Item.Type == ItemType.Ore) && (item.CurrentDura / 1000 > Required.Amount))
                            ...
                            DeleteItemDB(Info.Inventory[o].UniqueID, "inventory", true);
...
Enqueue(new S.DeleteItem { UniqueID = item.UniqueID, Count = count });
            ...
            string UpdateGuild = "UPDATE " + Settings.DBAccount + ".generalcount SET  NextGuildID = '" + Envir.NextGuildID + "'  WHERE IndexID = '1'; UPDATE " + Settings.DBAccount + ".characterinfo SET  GuildIndex = '" + Info.GuildIndex+ "'  WHERE IndexID = '" + Info.Index+ "'";
        ...
        public void GuildInvite(bool accept)
            ...
            string UpdateGuild = "UPDATE " + Settings.DBAccount + ".characterinfo SET  GuildIndex = '" + Info.GuildIndex + "'  WHERE IndexID = '" + Info.Index + "'";
        ...
        public void DepositTradeItem(int from, int to)
            ...
            if (Info.Trade[to] == null)
                ...
                string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET Store = 'Trade', Position = '" + to + "'  WHERE UniqueID = '" + Info.Trade[to].UniqueID + "'";
        ...
        public void RetrieveTradeItem(int from, int to)
            ...
            if (Info.Inventory[to] == null)
                ...
                string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET Store = 'Inventory', Position = '" + to + "'  WHERE UniqueID = '" + Info.Inventory[to].UniqueID + "'";
            ...
            if (CanTrade)
                ...
                for (int p = 0; p < 2; p++)
                    ...
                    for (int i = 0; i < TradePair[p].Info.Trade.Length; i++)
                        ...
                        DeleteItemDB(u.UniqueID, "inventory", false);
                    ...
                    if (TradePair[p].TradeGoldAmount > 0)
                        ...
                        //  string Update = "UPDATE " + Settings.DBAccount + ".account SET Gold = '" + TradePair[p].Account.Gold + "'  WHERE IndexID = '" + TradePair[o].Account.Index + "'";
        ...
        public void AcceptQuest(int index)
            ...
            DateTime theDate = quest.EndDateTime;
            DateTime theDate1 = quest.StartDateTime;
            CurrentQuests.Add(quest);
            string sqlquest = "INSERT INTO " + Settings.DBAccount + ".currentquests (ChName, IndexID, EndDateTime, StartDateTime, Quest, Type) VALUES ('" + Info.Name + "', '" + quest.Index + "','" + theDate.ToString("yyyy-MM-dd H:mm:ss").ToString() + "', '" + theDate1.ToString("yyyy-MM-dd H:mm:ss").ToString() + "', '" + quest.Info.Name + "', '" + quest.Info.Type.ToString() + "')";
            Envir.ConnectADB.Insert(sqlquest);

            if (quest.FlagTaskSet.Count != 0)
                {

                string sqlupdate = "UPDATE " + Settings.DBAccount + ".currentquests SET FlagTask = '1'  WHERE IndexID = '" + quest.Index + "' and ChName = '" + Info.Name + "'";

                Envir.ConnectADB.Update(sqlupdate);

                for (int i = 0; i < quest.FlagTaskSet.Count; i++)
                    {
                    string sqlFlag = "INSERT INTO " + Settings.DBAccount + ".flagtaskset (ChName, IndexID, Position, Value) VALUES ('" + Info.Name + "', '" + quest.Index + "', '" + i + "', '" + quest.FlagTaskSet[i] + "')";
                    Envir.ConnectADB.Insert(sqlFlag);
                    }

                }

            if (quest.ItemTaskCount.Count != 0)
                {

                string sqlupdate = "UPDATE " + Settings.DBAccount + ".currentquests SET ItemTask = '1'  WHERE IndexID = '" + quest.Index + "' and ChName = '" + Info.Name + "'";

                Envir.ConnectADB.Update(sqlupdate);

                for (int i = 0; i < quest.ItemTaskCount.Count; i++)
                    {
                    string sqlItem = "INSERT INTO " + Settings.DBAccount + ".itemtaskcount (ChName, IndexID, Position, Value) VALUES ('" + Info.Name + "', '" + quest.Index + "', '" + i + "', '" + quest.ItemTaskCount[i] + "')";

                    Envir.ConnectADB.Insert(sqlItem);
                    }

                }

            if (quest.KillTaskCount.Count != 0)
                {

                string sqlupdate = "UPDATE " + Settings.DBAccount + ".currentquests SET KillTask = '1'  WHERE IndexID = '" + quest.Index + "' and ChName = '" + Info.Name + "'";

                Envir.ConnectADB.Update(sqlupdate);

                for (int i = 0; i < quest.KillTaskCount.Count; i++)
                    {
                    string sqlKill = "INSERT INTO " + Settings.DBAccount + ".killtaskcount (ChName, IndexID, Position, Value) VALUES ('" + Info.Name + "', '" + quest.Index + "', '" + i + "', '" + quest.KillTaskCount[i] + "')";

                    Envir.ConnectADB.Insert(sqlKill);
                    }
        ...
        public void FinishQuest(int questIndex, int selectedItemIndex = -1)
            ...
            if (quest.Info.Type != QuestType.Repeatable)
                ...
                string sqlCount = "SELECT Count(*) FROM " + Settings.DBAccount + ".completedquests WHERE ChName = '" + Info.Name + "' AND CompletedQuests = '" + quest.Index + "'";

                if (Envir.ConnectADB.Count(sqlCount) > 0)
                    {

                    string UpdateItem = "UPDATE " + Settings.DBAccount + ".completedquests SET ChName = '" + Info.Name + "', CompletedQuests = '" + quest.Index + "', Quest = '" + quest.Info.Name + "', Type = '" + quest.Info.Type.ToString() + "' WHERE ChName = '" + Info.Name + "' AND CompletedQuests = '" + quest.Index + "', Finish = '" + theDate.ToString("yyyy-MM-dd H:mm:ss").ToString() + "'";
                    ...
                    string sql = "INSERT INTO " + Settings.DBAccount + ".completedquests (ChName, CompletedQuests, Quest, Type, Finish) VALUES ('" + Info.Name + "', '" + quest.Index + "', '" + quest.Info.Name + "', '" + quest.Info.Type.ToString() + "', '" + theDate.ToString("yyyy-MM-dd H:mm:ss").ToString() + "')";
            ...
            string sqlDelete = "DELETE FROM " + Settings.DBAccount + ".currentquests WHERE IndexID = '" + quest.Index + "' and ChName = '" + Info.Name + "'; DELETE FROM " + Settings.DBAccount + ".flagtaskset WHERE IndexID = '" + quest.Index + "' and ChName = '" + Info.Name + "'; DELETE FROM " + Settings.DBAccount + ".itemtaskcount WHERE IndexID = '" + quest.Index + "' and ChName = '" + Info.Name + "'; DELETE FROM " + Settings.DBAccount + ".killtaskcount WHERE IndexID = '" + quest.Index + "' and ChName = '" + Info.Name + "'";
        ...
        public bool CheckNeedQuestItem(UserItem item, bool gainItem = true)
            ...
            foreach (QuestProgressInfo quest in CurrentQuests.
                ...
                if (gainItem)
                    ...
                    for (int i = 0; i < quest.ItemTaskCount.Count; i++)
                        {
                        string Update = "UPDATE " + Settings.DBAccount + ".itemtaskcount SET Value = '" + quest.ItemTaskCount[i] + "'  WHERE Position = '" + i + "' AND IndexID = '" + quest.Index + "' AND ChName = '" + Info.Name + "'";
        ...
        public bool CheckNeedQuestFlag(int flagNumber)
            ...
            foreach (QuestProgressInfo quest in CurrentQuests.
                ...
                for (int i = 0; i < quest.FlagTaskSet.Count; i++)
                    {
                    string Update = "UPDATE " + Settings.DBAccount + ".flagtaskset SET Value = '" + Convert.ToInt32(quest.FlagTaskSet[i]) + "'  WHERE Position = '" + i + "' AND IndexID = '" + quest.Index + "' AND ChName = '" + Info.Name + "'";
        ...
        public void CheckNeedQuestKill(MonsterInfo mInfo)
            ...
            foreach (QuestProgressInfo quest in CurrentQuests.
                ...
                for (int i = 0; i < quest.KillTaskCount.Count; i++)
                    {
                    string Update = "UPDATE " + Settings.DBAccount + ".killtaskcount SET Value = '" + quest.KillTaskCount[i] + "'  WHERE Position = '" + i + "' AND IndexID = '" + quest.Index + "' AND ChName = '" + Info.Name + "'";
        ...
        public void RecalculateQuestBag()
            ...
            for (int i = Info.QuestInventory.Length - 1; i >= 0; i--)
                ...
                if (!itemRequired)
                    {
                    DeleteItemDB(Info.QuestInventory[i].UniqueID, "inventory", true);
        ...
        public void SendUpdateQuest(QuestProgressInfo quest, QuestState state, bool trackQuest = false)
            ...
            string Update = "UPDATE " + Settings.DBAccount + ".currentquests SET EndDateTime = '" + theDate.ToString("yyyy-MM-dd H:mm:ss").ToString() + "'  WHERE ChName = '" + Info.Name + "' And IndexID = '" + quest.Index + "'";
        ...
        public void SendMail(string name, string message)
            ...
            if (mail.Items.Count > 0)
                ...
                for (int i = 0; i < mail.Items.Count; i++)
                    {
                    GainAddItemDB(mail.Items[i], "mailitems", "Mail", 0, mail.MailID);
                    }
                }
            }
        ...
        public void SendMail(string name, string message, uint gold, ulong[] items, bool stamped)
            ...
            CharacterInfo player = Envir.GetCharacterInfoDB(name);
            ...
            if (stamped)
                ...
                for (int i = 0; i < Info.Inventory.Length; i++)
                    ...
                    if (item.Count > 1)
                        ...
                        {
                        Info.Inventory[i] = null;
                        DeleteItemDB(item.UniqueID, "inventory", true);
                        }
            ...
            for (int j = 0; j < (hasStamp ? 5 : 1); j++)
                ...
                for (int i = 0; i < Info.Inventory.Length; i++)
                    ...
                    DeleteItemDB(Info.Inventory[i].UniqueID, "inventory", false);
        ...
        public void ReadMail(ulong mailID)
            ...
            DateTime theDate = mail.DateOpened;

            string Update = "UPDATE " + Settings.DBAccount + ".mail SET DateOpened = '" + theDate.ToString("yyyy-MM-dd H:mm:ss").ToString() + "'  WHERE MailID = '" + mail.MailID + "'";
        ...
        public void CollectMail(ulong mailID)
            ...
            if (mail.Items.Count > 0)
                ...
                for (int i = 0; i < mail.Items.Count; i++)
                    ...
                    DeleteItemDB(mail.Items[i].UniqueID, "mailitems", false);
                    }
                }
        ...
        public void DeleteMail(ulong mailID)
            ...
            string sqlDelete = "DELETE FROM " + Settings.DBAccount + ".mail WHERE MailID = '" + mail.MailID + "'; DELETE FROM " + Settings.DBAccount + ".mailitems WHERE MailID = '" + mail.MailID + "'";
        ...
        public void LockMail(ulong mailID, bool lockMail)
            ...
            string Update = "UPDATE " + Settings.DBAccount + ".mail SET Locked = '" + Convert.ToInt32(mail.Locked) + "'  WHERE MailID = '" + mail.MailID + "'";
        ...
        public void ReleaseIntelligentCreature(IntelligentCreatureType pType, bool doUpdate = true)
            ...
            for (int i = 0; i < Info.IntelligentCreatures.Count; i++)
                ...
                string sqlDelete = "DELETE FROM " + Settings.DBAccount + ".intelligentcreatures WHERE ChName = '" + Info.Name + "' And PetType = '" + Convert.ToInt32(Info.IntelligentCreatures[i].PetType) + "'";
            ...
            for (int i = 0; i < Info.IntelligentCreatures.Count; i++)
                {
                if (Info.IntelligentCreatures[i].SlotIndex != i)
                    {
                    string Update = "UPDATE " + Settings.DBAccount + ".intelligentcreatures SET SlotIndex = '" + i + "'  WHERE ChName = '" + Info.Name + "' And PetType = '" + Convert.ToInt32(Info.IntelligentCreatures[i].PetType) + "'";
                    Envir.ConnectADB.Update(Update);
                    }
        ...
        public void IntelligentCreatureGainPearls(int amount)
            ...
            if (Info.PearlCount > int.MaxValue)
                {
                Info.PearlCount = int.MaxValue;
                }

            string UpdatePearl = "UPDATE " + Settings.DBAccount + ".characterinfo SET PearlCount = '" + Info.PearlCount + "'  WHERE IndexID = '" + Info.Index + "'";

            Envir.ConnectADB.Update(UpdatePearl);
        ...
        public void AddFriend(string name, bool blocked = false)
            {
            CharacterInfo info = Envir.GetCharacterInfoDB(name);
            ...
            if (Info.Friends.Count >= 50)
                {
                ReceiveChat("You have reached the maximum limit of friends", ChatType.System);
                return;
                }
            ...
            string sql = "INSERT INTO " + Settings.DBAccount + ".friends (IndexID, ChName, Blocked, Memo, Name) VALUES ('" + friend.Index + "', '" + Info.Name + "', '" + Convert.ToInt32(friend.Blocked) + "', '" + friend.Memo + "','" + friend.Name + "')";
        ...
        public void RemoveFriend(int index)
            ...
            string sqlDelete = "DELETE FROM " + Settings.DBAccount + ".friends WHERE IndexID = '" + friend.Index + "' AND ChName = '" + Info.Name + "'";
        ...
        public void AddMemo(int index, string memo)
            ...
            string Update = "UPDATE " + Settings.DBAccount + ".friends SET Memo = '" + friend.Memo + "'  WHERE IndexID = '" + friend.Index + "' AND ChName = '" + Info.Name + "'";
        ...
        public void DepositRefineItem(int from, int to)
            ...
            if (Info.Refine[to] == null)
                ...
                string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET Store = 'Refine', Position = '" + to + "'  WHERE UniqueID = '" + Info.Refine[to].UniqueID + "'";
        ...
        public void RefineItem(ulong uniqueID)
            ...
            string UpdateItem = "UPDATE " + Settings.DBAccount + ".inventory SET Store = 'CurrentRefine'  WHERE UniqueID = '" + Info.Inventory[index].UniqueID + "'; UPDATE " + Settings.DBAccount + ".characterinfo SET CollectTime = '" + Info.CollectTime + "'  WHERE IndexID = '" + Info.Index + "'";
            Envir.ConnectADB.Update(UpdateItem);
            ...
            for (int i = 0; i < Info.Refine.Length; i++)
                ...
                if (Ingredient.Info.Type == ItemType.Weapon)
                    {
                    DeleteItemDB(Info.Refine[i].UniqueID, "inventory", true);
            ...
            if ((TotalDC == 0) && (TotalMC == 0) && (TotalSC == 0))
                ...
                string UpdateRefine1 = "UPDATE " + Settings.DBAccount + ".inventory SET RefineAdded = '" + Info.CurrentRefine.RefineAdded + "', RefinedValue = '" + Convert.ToInt32(Info.CurrentRefine.RefinedValue) + "'  WHERE UniqueID = '" + Info.CurrentRefine.UniqueID + "'";

                Envir.ConnectADB.Update(UpdateRefine1);
            ...
            if (OreAmount == 0)
                ...
                string UpdateRefine3 = "UPDATE " + Settings.DBAccount + ".inventory SET RefineAdded = '" + Info.CurrentRefine.RefineAdded + "', RefinedValue = '" + Convert.ToInt32(Info.CurrentRefine.RefinedValue) + "'  WHERE UniqueID = '" + Info.CurrentRefine.UniqueID + "'";

                Envir.ConnectADB.Update(UpdateRefine3);
            ...
            string UpdateRefine = "UPDATE " + Settings.DBAccount + ".inventory SET RefineAdded = '" + Info.CurrentRefine.RefineAdded + "', RefinedValue = '" + Info.CurrentRefine.RefineAdded + "'  WHERE UniqueID = '" + Info.CurrentRefine.UniqueID + "'";

            Envir.ConnectADB.Update(UpdateRefine);
        ...
        public void CollectRefine()
            ...
            DeleteItemDB(Info.CurrentRefine.UniqueID, "inventory", true);
        ...
        public void CheckRefine(ulong uniqueID)
            ...
            if ((Info.Inventory[index].RefinedValue == RefinedValue.DC) && (Info.Inventory[index].RefineAdded > 0))
                ...
                string UpdateRefineDC = "UPDATE " + Settings.DBAccount + ".inventory SET RefineAdded = '" + Info.Inventory[index].RefineAdded + "', RefinedValue = '" + Info.Inventory[index].RefineAdded + "', DC = '" + Info.Inventory[index].DC + "'  WHERE UniqueID = '" + Info.Inventory[index].UniqueID + "'";

                Envir.ConnectADB.Update(UpdateRefineDC);
                ...
                string UpdateRefineMC = "UPDATE " + Settings.DBAccount + ".inventory SET RefineAdded = '" + Info.Inventory[index].RefineAdded + "', RefinedValue = '" + Info.Inventory[index].RefineAdded + "', MC = '" + Info.Inventory[index].MC + "'  WHERE UniqueID = '" + Info.Inventory[index].UniqueID + "'";

                Envir.ConnectADB.Update(UpdateRefineMC);
                ...
                string UpdateRefineSC = "UPDATE " + Settings.DBAccount + ".inventory SET RefineAdded = '" + Info.Inventory[index].RefineAdded + "', RefinedValue = '" + Info.Inventory[index].RefineAdded + "', SC = '" + Info.Inventory[index].SC + "'  WHERE UniqueID = '" + Info.Inventory[index].UniqueID + "'";

                Envir.ConnectADB.Update(UpdateRefineSC);
        ...
        public void NPCDivorce()
            ...
            Info.MarriedName = "";
            DateTime theDate1 = Info.MarriedDate;
            ...
            if (Info.Equipment[(int)EquipmentSlot.RingL] != null)
                ...
                Update = "UPDATE  " + Settings.DBAccount + ".inventory SET WeddingRing = '-1' WHERE UniqueID = '" + Info.Equipment[(int)EquipmentSlot.RingL].UniqueID + "'";
                Envir.ConnectADB.Update(Update);
            ...
            Lover.MarriedName = "";
            DateTime theDate = Lover.MarriedDate;

            if (Lover.Equipment[(int)EquipmentSlot.RingL] != null)
                {
                Lover.Equipment[(int)EquipmentSlot.RingL].WeddingRing = -1;

                Update = "UPDATE  " + Settings.DBAccount + ".inventory SET WeddingRing = '-1' WHERE UniqueID = '" + Lover.Equipment[(int)EquipmentSlot.RingL].UniqueID + "'";
                Envir.ConnectADB.Update(Update);
                }


            Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET Married = '" + Lover.Married + "', MarriedName = '" + Lover.MarriedName + "', MarriedDate =  '" + theDate.ToString("yyyy-MM-dd H:mm:ss") + "'  WHERE IndexID = '" + Lover.Index + "'; UPDATE " + Settings.DBAccount + ".characterinfo SET Married = '" + Info.Married + "', MarriedName = '" + Info.MarriedName + "', MarriedDate =  '" + theDate1.ToString("yyyy-MM-dd H:mm:ss") + "'  WHERE IndexID = '" + Info.Index + "'";
        ...
        public void MakeWeddingRing()
            ...
            if (CheckMakeWeddingRing())
                ...
                string Update = "UPDATE  " + Settings.DBAccount + ".inventory SET WeddingRing = '" + Info.Married + "' WHERE UniqueID = '" + Info.Equipment[(int)EquipmentSlot.RingL].UniqueID + "'";
        ...
        public void ReplaceWeddingRing(ulong uniqueID)
            ...
            Update = "UPDATE  " + Settings.DBAccount + ".inventory SET Store = 'Equipment', Position = '" + (int)EquipmentSlot.RingL + "', WeddingRing = '" + Info.Equipment[(int)EquipmentSlot.RingL].WeddingRing + "'  WHERE UniqueID = '" + Info.Equipment[(int)EquipmentSlot.RingL].UniqueID + "'; UPDATE  " + Settings.DBAccount + ".inventory SET Store = 'Inventory', Position = '" + index + "', WeddingRing = '" + Info.Inventory[index].WeddingRing + "'  WHERE UniqueID = '" + Info.Inventory[index].UniqueID + "'";
        ...
        public void MarriageReply(bool accept)
            ...
            MarriageProposal.Info.MarriedName = Info.Name;
            DateTime theDate = MarriageProposal.Info.MarriedDate;

            Info.Married = MarriageProposal.Info.Index;
            Info.MarriedDate = DateTime.Now;
            Info.MarriedName = MarriageProposal.Info.Name;
            DateTime theDate1 = Info.MarriedDate;

            string Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET Married = '" + MarriageProposal.Info.Married + "', MarriedName = '" + MarriageProposal.Info.MarriedName + "', MarriedDate =  '" + theDate.ToString("yyyy-MM-dd H:mm:ss") + "'  WHERE IndexID = '" + MarriageProposal.Info.Index + "'; UPDATE " + Settings.DBAccount + ".characterinfo SET Married = '" + Info.Married + "', MarriedName = '" + Info.MarriedName + "', MarriedDate =  '" + theDate1.ToString("yyyy-MM-dd H:mm:ss") + "'  WHERE IndexID = '" + Info.Index + "'";
        ...
        public void DivorceReply(bool accept)
            ...
            DivorceProposal.Info.MarriedName = "";
            DateTime theDate = DivorceProposal.Info.MarriedDate;
            ...
            if (DivorceProposal.Info.Equipment[(int)EquipmentSlot.RingL] != null)
                ...
                Update = "UPDATE  " + Settings.DBAccount + ".inventory SET WeddingRing = '-1' WHERE UniqueID = '" + DivorceProposal.Info.Equipment[(int)EquipmentSlot.RingL].UniqueID + "'";
                Envir.ConnectADB.Update(Update);
            ...
            Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET Married = '" + DivorceProposal.Info.Married + "', MarriedName = '" + DivorceProposal.Info.MarriedName + "', MarriedDate =  '" + theDate.ToString("yyyy-MM-dd H:mm:ss") + "'  WHERE IndexID = '" + DivorceProposal.Info.Index + "'; UPDATE " + Settings.DBAccount + ".characterinfo SET Married = '" + Info.Married + "', MarriedName = '" + Info.MarriedName + "', MarriedDate =  '" + theDate1.ToString("yyyy-MM-dd H:mm:ss") + "'  WHERE IndexID = '" + Info.Index + "'";
        ...
        public void GetRelationship(bool CheckOnline = true)
            ...
            if (Info.Married == 0)
                ...
                {
                CharacterInfo Lover = Envir.GetCharacterInfoDB(Info.Married);

                PlayerObject player = Envir.GetPlayer(Lover.Name);

                if (player == null)
                    Enqueue(new S.LoverUpdate { Name = Info.MarriedName, Date = Info.MarriedDate, MapName = "", MarriedDays = (short)(DateTime.Now - Info.MarriedDate).TotalDays });
                else
                    {
                    Enqueue(new S.LoverUpdate { Name = Info.MarriedName, Date = Info.MarriedDate, MapName = player.CurrentMap.Info.Title, MarriedDays = (short)(DateTime.Now - Info.MarriedDate).TotalDays });
                    if (CheckOnline)
                        {
                        player.GetRelationship(false);
        ...
        public void MentorBreak(bool Force = false)
            ...
            // CharacterInfo Mentor = Envir.GetCharacterInfo(Info.Mentor);
            CharacterInfo Mentor = Envir.GetCharacterInfoDB(Info.Mentor);
            PlayerObject Player = Envir.GetPlayer(Mentor.Name);

            if (Force)
                {
                Info.MentorDate = DateTime.Now.AddDays(Settings.MentorLength);
                DateTime theDate = Info.MentorDate;

                string UpdateDate = "UPDATE " + Settings.DBAccount + ".characterinfo SET MentorDate = '" + theDate.ToString("yyyy-MM-dd H:mm:ss") + "' WHERE IndexID = '" + Info.Index + "'";

                Envir.ConnectADB.Update(UpdateDate);
            ...
            if (Player != null)
                ...
                if (Mentor.isMentor && Mentor.MentorExp > 0)
                    ...
                    string UpdateEXP = "UPDATE " + Settings.DBAccount + ".characterinfo SET Mentor = '" + Mentor.Experience + "'  WHERE IndexID = '" + Mentor.Index + "'";
                    Envir.ConnectADB.Update(UpdateEXP);
            ...
            string Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET Mentor = '" + Info.Mentor + "', MentorName = '" + Info.MentorName + "',isMentor = '" + Convert.ToInt32(Info.isMentor) + "', MentorExp = '" + Info.MentorExp + "'  WHERE IndexID = '" + Info.Index + "'; UPDATE " + Settings.DBAccount + ".characterinfo SET Mentor = '" + Mentor.Mentor + "', MentorName = '" + Mentor.MentorName + "', isMentor = '" + Convert.ToInt32(Mentor.isMentor) + "', MentorExp = '" + Mentor.MentorExp + "'  WHERE IndexID = '" + Mentor.Index + "'";
            Envir.ConnectADB.Update(Update);
        ...
        public void MentorReply(bool accept)
            ...
            if (Student == null)
                ...
                Student.Info.MentorName = Info.Name;
                DateTime theDate = Student.Info.MentorDate;

                Info.Mentor = Student.Info.Index;
                Info.isMentor = true;
                Info.MentorDate = DateTime.Now;
                Info.MentorName = Student.Info.Name;
                DateTime theDate1 = Info.MentorDate;

                string Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET Mentor = '" + Info.Mentor + "', MentorName = '" + Info.MentorName + "', MentorDate =  '" + theDate.ToString("yyyy-MM-dd H:mm:ss") + "', isMentor = '" + Convert.ToInt32(Info.isMentor) + "'  WHERE IndexID = '" + Info.Index + "'; UPDATE " + Settings.DBAccount + ".characterinfo SET Mentor = '" + Student.Info.Mentor + "', MentorName = '" + Student.Info.MentorName + "', MentorDate =  '" + theDate1.ToString("yyyy-MM-dd H:mm:ss") + "', isMentor = '" + Convert.ToInt32(Student.Info.isMentor) + "'  WHERE IndexID = '" + Student.Info.Index + "'";
        ...
        public void LogoutMentor()
            ...
            if (!Info.isMentor)
                ...
                string Update = "UPDATE " + Settings.DBAccount + ".characterinfo SET MentorExp = '" + Mentor.MentorExp + "' WHERE IndexID = '" + Mentor.Index + "'";
                Envir.ConnectADB.Update(Update);
                }
        ...
        public void GameShopStock(GameShopItem item)
            ...
            if (item.iStock) //Invididual Stock
                {
                Info.GSpurchases.TryGetValue(item.GIndex, out purchased);
                }
            else //Server Stock
                {
                Envir.GameshopLog.TryGetValue(item.GIndex, out purchased);
                }
            ...
            if (item.Stock - purchased >= 0)
                ...
                string Update = "UPDATE " + Settings.DBServer + ".gameshopitem SET Stock = '" + StockLevel + "' WHERE GIndex = '" + item.GIndex + "'";
        ...
        public void GameshopBuy(int GIndex, byte Quantity)
            ...
            if (Product.Stock != 0)
                ...
                if (Product.iStock) //Invididual Stock
                    {
                    Info.GSpurchases.TryGetValue(Product.GIndex, out purchased);
                    }
                else //Server Stock
                    {
                    Envir.GameshopLog.TryGetValue(Product.GIndex, out purchased);
                    }
            ...
            if (canAfford)
                ...
                string Update = "UPDATE " + Settings.DBAccount + ".account SET Credit = '" + Account.Credit + "', Gold= '" + Account.Gold + "'  WHERE IndexID = '" + Account.Index + "'";
                ...
                if (Product.iStock && Product.Stock != 0)
                    ...
                    if (Purchased == 0)
                        ...
                        string sql = "INSERT INTO " + Settings.DBAccount + ".gspurchases (ChName, Key_, Value) VALUES ('" + Info.Name + "', '" + Product.GIndex + "', '" + Quantity + "')";
                        ...
                        string UpdateGS = "UPDATE " + Settings.DBAccount + ".gspurchases SET Value = '" + Info.GSpurchases[Product.GIndex] + "'  WHERE ChName = '" + Info.Name + "' And Key_ = '"+ Product.GIndex + "'";

                        Envir.ConnectADB.Update(UpdateGS);
                ...
                if (Purchased == 0)
                    ...
                    string sql = "INSERT INTO " + Settings.DBAccount + ".gameshoplog (Key_, Value) VALUES ('" + Product.GIndex + "', '" + Quantity + "')";
                    ...
                    {
                    Envir.GameshopLog[Product.GIndex] += Quantity;
                    string UpdateLog = "UPDATE " + Settings.DBAccount + ".gameshoplog SET Value = '" + Envir.GameshopLog[Product.GIndex] + "'  WHERE  Key_ = '" + Product.GIndex + "'";

                    Envir.ConnectADB.Update(UpdateLog);
                    }
